...
import type { PlusTooltipPlacement, PlusTooltipTrigger } from './types';
 ...
 */
@Element()
export class PlusTooltip extends PlusCore {
	/**
	...
	initialize() {
		...
		this.events(false).forEach(([type, handler]) => {
			...
			on(this.$activator, type as any, handler as any);
		});
	...
	terminate() {
		...
		this.events(true).forEach(([type, handler]) => {
			// biome-ignore lint: TODO
			off(this.$activator, type as any, handler as any);
		});
	}

	events(all: boolean) {
		return this.EVENTS.filter((row) => all || [this.trigger].flat().includes(row[0])).map((row) =>
			row.slice(1)
		);
	}
